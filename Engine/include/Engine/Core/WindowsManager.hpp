...
struct WindowsManager {
	...
	DLLEXPORT bool SetWindowFullscreen(size_t winId, bool) const;
	...
	DLLEXPORT bool CenterWindow(uint32_t winId, uint32_t screenId = 0) const;
	DLLEXPORT bool SetWindowTopLeftCorner(size_t winId, uint32_t x, uint32_t y) const;
